...
def plot_performance_ndjson(json_file, parameter):
    groups = defaultdict(lambda: {"naive": [], "winograd": [], "speedup": []})
    ...
    with open(json_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                ...
                groups[x_value]["speedup"].append(item.get("speedup", 0))
    ...
    speedup_avg = np.array([np.mean(groups[x]["speedup"]) for x in sorted_x])
    
    mean_speedup = float(np.mean(speedup_avg))
    ...
    ax2.plot(x_arr, speedup_avg, 'g-o', markersize=4, alpha=0.4, label='Speedup')
    
    ax2.axhline(y=mean_speedup, color='darkgreen', linestyle='--', linewidth=2, 
                label=f'Average Speedup: {mean_speedup:.2f}x')
    
    ax2.set_title('Winograd Speedup Factor', fontsize=14)
