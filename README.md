...
Реализованы наивный алгоритм, im2col алгоритм и алгоритм Винограда (ядро 3 x 3, выход 2 x 2) свертки тензора. Далее опишем более подробно каждый из подходов. Можете посмотреть как я их реализовал [здесь](tensor/src/tensor_conv.cppm).
Ядра для видеокарточки можно посмотреть [здесь](tensor/kernels/).
...
Название алгоритма происходит от *image to column* — изображение в столбец. Идея состоит в том, чтобы
свести свёртку к обычному матричному умножению, предварительно переупорядочив данные.

Для каждого положения тайла (то есть для каждого выходного пикселя) тайл входного изображения
«разворачивается» в плоский вектор-строку.

Конкретно в коде развёртка тайла в строку выглядит так:
...
Двумерный тайл $kH \times kW$ превращается в одномерный массив длиной $kH \cdot kW$, где
элемент $(tile\_y,\, tile\_x)$ отображается в индекс $tile\_y \cdot kW + tile\_x$.

Далее происходит поэлементное умножение развёрнутого тайла на развёрнутое ядро и суммирование
результата — это и есть скалярное произведение строки $\text{col}$ на столбец $\text{K}$. В коде
этот шаг ускорен AVX-инструкциями: за одну итерацию обрабатывается 8 элементов одновременно:
...
accumulated += simdRes[0] + simdRes[1] + ... + simdRes[7];
```

Результат накапливается по всем каналам и записывается в выходной тензор.

Главное отличие от наивного алгоритма — не в количестве арифметических операций (оно то же самое),
а в **паттерне доступа к памяти**. Наивный алгоритм читает тайл напрямую из входного тензора,
где строки тайла разделены шагом $iW$ элементов — это некэшируемый доступ. Im2Col сначала
копирует тайл в непрерывный буфер `inputTile`, после чего умножение идёт по последовательным
адресам памяти, что даёт хорошую утилизацию кэша и позволяет эффективно применять SIMD-инструкции.

На GPU реализация такого алгоритма никакого прироста не даст в силу и так высокого параллелизма.
...
<img src="md_sources/win_vs_im2col_vs_naive(batchSize).png" width="800" alt="Winograd vs Naive Performance">

Im2Col сильно выигрывает от скажем так анлога GEMM операции - AVX инструкций. За счет кэш-френдли идеалогии алгоритма и параллельной обработки 8 float, Im2Col очень сильно уходит в отрыв.
...
Ядра для `naive`, `Winograd` и `im2col` я писал на OpenCL.

<img src="md_sources/winograd_vs_im2col_vs_naive(batchSize)_gpu.png" width="800" alt="Winograd vs Naive Performance">

Im2Col показал себя не очень хорошо. Я думаю это связано с небольшими размерами картинок и огромной 
прожорливости по памяти. Гонять на видеокарту и обратно глобальную память оказалось достаточно трудоемко, 
поэтому выигрыша нет. Возможно стоит пересмотреть работу с памятью и работать с локальной, а не глобальной 
памятью.
